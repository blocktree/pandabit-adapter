...
func (decoder *TransactionDecoder) CreatePBRawTransaction(wrapper openwallet.WalletDAI, rawTx *openwallet.RawTransaction) error {
	...
	for _, a := range addressesBalanceList {
		if a.Balance.Cmp(amount) < 0 {
			...
			if count.Cmp(amount) >= 0 {
				...
				log.Error("The PB of the account is enough,"+
...
func (decoder *TransactionDecoder) SignPBRawTransaction(wrapper openwallet.WalletDAI, rawTx *openwallet.RawTransaction) error {
...
func (decoder *TransactionDecoder) VerifyPBRawTransaction(wrapper openwallet.WalletDAI, rawTx *openwallet.RawTransaction) error {
